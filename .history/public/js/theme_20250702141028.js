  ...
  if ($trafficChannels) {
    ...
    var getDefaultOptions = function getDefaultOptions() {
      return {
        ...
        xAxis: {
          type: 'category',
          data: utils.getPastDates(10).map(function (date) {
            return window.dayjs(date).format('DD MMM, YYYY');
          }),
          axisLine: {
            show: false
...
var echartsUnresolvedTicketsInit = function echartsUnresolvedTicketsInit() {
  ...
  if ($unresolvedTickets) {
    ...
    var xAxisData = ['公立', '民营', '综合', '中医', '中西医'];
    var data1 = [20, 18, 15, 20, 12];
    var data2 = [30, 20, 20, 25, 20];
    var data3 = [35, 32, 40, 50, 30];
    var data4 = [15, 25, 20, 18, 10];
    var emphasisStyle = {
      itemStyle: {
        shadowColor: utils.rgbaColor(utils.getColor('dark'), 0.3)
      }
