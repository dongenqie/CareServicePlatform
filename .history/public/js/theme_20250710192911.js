...
var dropzoneInit = function dropzoneInit() {
  ...
  // 只初始化没有 data-no-init 属性的 dropzone 表单
  var dropzones = Array.from(document.querySelectorAll(Selector.DROPZONE))
  .filter(el => !el.hasAttribute('data-no-init'));
  if (dropzones.length > 0) {
    dropzones.forEach(function (item) {
      // 如果该元素已有 Dropzone 实例，则跳过
      if (item.dropzone) {
        console.log("Dropzone 已经初始化，跳过此元素");
        return;
      }
      ...
      item.dropzone = dropzone; // 添加 Dropzone 实例到元素上，避免下次重复初始化
...
var navbarDarkenOnScroll = function navbarDarkenOnScroll() {
  ...
  var toggleThemeClass = function toggleThemeClass(theme) {
    if (theme === 'dark') {
      navbar.classList.remove('navbar-vibrant');
...
//           transitionDuration: 0,
//           formatter: function formatter(params) {
//             return "".concat(params[0].axisValue, " - ").concat(params[0].value, "元");
//           },
//           textStyle: {
//             fontWeight: 500,
//             fontSize: 12,
...
//     var data1 = [45, 35, 55, 55, 55];
//     var data2 = [58, 42, 65, 65, 65];
//     var data3 = [38, 25, 42, 42, 42];
//     var data4 = [62, 45, 75, 75, 75];
//     var emphasisStyle = {
//       itemStyle: {
//         shadowColor: utils.rgbaColor(utils.getColor('dark'), 0.3),
