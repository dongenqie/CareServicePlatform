...
const handleFileChange = (event) => {
  // 如果你使用的是 Dropzone，确保 file 通过 Dropzone 获取
  if (myDropzone) {
      const file = myDropzone.files[0];  // 获取第一个文件
      file.value = file;  // 更新 file 变量
...
const submitFile = async () => {
  ...
  console.log("Attribute 1:", attributeName1.value);  // 调试信息
  console.log("Attribute 2:", attributeName2.value);  // 调试信息
  ...
  myDropzone = new Dropzone('#my-awesome-dropzone', {
      ...
      init: function() {
          this.on('addedfile', (file) => {
              ...
              // 手动设置文件
              file.value = file;  // 将选择的文件赋给 file.value
          });
          this.on('removedfile', () => {
              file.value = null;  // 文件被移除时清空值
          });
      }
  });
