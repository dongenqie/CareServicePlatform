...
const submitFile = async () => {
  if (!file.value || !attributeName1.value || !attributeName2.value || !directTime.value) {
    alert('请完整填写所有信息');
    return;
  }
  ...
  try {
    ...
    // 使用你已有的 trendChartInit 方法来初始化图表
    trendChartInit(data.scatterVO, data.trendVO);  // 将返回的数据传递给 trendChartInit
  } catch (error) {
