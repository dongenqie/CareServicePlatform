                        ...
                        <div class="card-body">
                          <transition name="fade" @before-enter="beforeEnter" @enter="enter" @leave="leave">
                            <div v-if="analysisResults" class="text-center">
                              <p class="text-white">经分析，{{ comparativeData.horizonVO.meanResult }},与因变量{{ correlationData.name2 }}的相关系数为 <strong>{{ correlationData.correlation }}</strong>, <strong>{{ correlationData.conform ? '呈' : '非' }}</strong> 线性相关</p>
                            </div>
                          </transition>
                        </div>
                      </div>
