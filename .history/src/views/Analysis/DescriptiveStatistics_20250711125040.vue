                        ...
                        <div class="card-body">
                          <transition name="fade" @before-enter="beforeEnter" @enter="enter" @leave="leave">
                            <div v-if="analysisResults" class="text-center">
                              <p class="text-white">经分析，数据的中位数为 <strong>{{ median }}</strong>,平均值为 <strong>{{ average }}</strong>,标准差为 <strong>{{ stdDev }}</strong>, <strong>{{ normal ? '符合正太分布' : '非正太分布' }}</strong>, <strong>{{ index ? '符合指数分布' : '非指数分布' }}</strong></p>
                            </div>
                          </transition>
                        </div>
                      </div>
