...
const handleSubmit = async () => {
  if (!file.value || !attributeName1.value || !attributeName2.value) {
    alert('请上传文件并指定自变量和因变量');
    return;
  }

  try {
    const result = await getCorrelation(file.value, attributeName1.value, attributeName2.value);
    correlationData.value = result.correlationVO; // 相关性数据
    scatterData.value = result.scatterVO.data;  // 散点图数据

    // 更新散点图
    updateChart(scatterData.value);
  } catch (error) {
...
const updateChart = (data) => {
  const chart = window.echarts.getInstanceByDom(document.querySelector('.echart-analysis-correlation'));
  const options = {
    tooltip: {
      trigger: 'item',
      axisPointer: {
    ...
    xAxis: {
      type: 'value',
      name: attributeName1.value,
    },
    yAxis: {
      type: 'value',
      name: attributeName2.value,
    },
    series: [{
      data: data,
      type: 'scatter',
...
const submitFile = async () => {
  try {
    const data = await getCorrelation(file.value, attributeName1.value, attributeName2.value);
    scatterData.value = data.scatterVO.data; // 获取散点图数据

    // 调用图表初始化函数来渲染图表
    correlationChartInit(scatterData.value);
  } catch (error) {
          ...
          <div class="row g-3">
                  ...
                  <div class="row flex-between-end">
                    ...
                    <div class="col-auto ms-auto">
                      <div class="nav nav-pills nav-pills-falcon flex-grow-1" role="tablist">
                        <button class="btn btn-sm active" data-bs-toggle="pill" data-bs-target="#dom-ae60896f-e83f-4d14-9861-a8cd380680c1" type="button" role="tab" aria-controls="dom-ae60896f-e83f-4d14-9861-a8cd380680c1" aria-selected="true" id="tab-dom-ae60896f-e83f-4d14-9861-a8cd380680c1">Preview</button>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="row g-3">
                <div class="card mb-3">
                  ...
                  <div class="card-bottom mt-2 mb-3">
                    <button class="btn btn-falcon-default btn-sm me-2 float-end" type="button" @click="handleSubmit">提交</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div v-if="correlationData" class="col-lg-12">
              <div class="card mb-3">
                ...
                <div class="card-body">
                  <p><strong>自变量:</strong> {{ correlationData.name1 }}</p>
                  <p><strong>因变量:</strong> {{ correlationData.name2 }}</p>
                  <p><strong>相关性系数:</strong> {{ correlationData.correlation }}</p>
