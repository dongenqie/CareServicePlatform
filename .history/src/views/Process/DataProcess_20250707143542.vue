...
const duplicateColsText    = ref('')   // 查重列逗号分隔
...
async function submitFile() {
  ...
  try {
    const fileToUpload = myDropzone.files[0]
    // 假设 getDescriptiveStatistics 接受 File 对象
...
async function handleCleanPreview() {
  ...
  console.log('清洗预览数据：', res.data)
  // TODO: 把 res.data 渲染到表格里
}
...
async function handleRemoveDupPreview() {
  ...
  const fileObj = myDropzone.files[0]
  const pks = duplicatePKText.value.split(',').map(s => s.trim()).filter(Boolean)
  const cols = duplicateColsText.value.split(',').map(s => s.trim()).filter(Boolean)
  const res = await removeDuplicates({
    file: fileObj,
    format: cleanFormat.value,
    primaryKeyColumns: pks,
    duplicateCheckColumns: cols
  })
  console.log('去重预览数据：', res.data)
  // TODO: 渲染到表格
}
...
async function handleCleanExport() {
  ...
  await cleanExport({
    ...
    targetColumn: targetColumn.value,
    dependentColumns: deps,
    exportFormat: tableFormat.value,
    filename: tableFilename.value
  })
}

async function handleRemoveDupExport() {
  if (!myDropzone || myDropzone.files.length === 0) {
    return alert('请先上传文件')
  }
  const fileObj = myDropzone.files[0]
            ...
            <div class="content">
                    ...
                    <div class="row g-0 h-100">
                        ...
                        <div class="card bg-body-tertiary dark__bg-opacity-50 shadow-none">
                            ...
                            <div class="ms-n3">
                                ...
                                <h4 class="mb-0 text-primary fw-bold">健康大数据中心<span class="text-info fw-medium">数据处理实验室</span></h4>
                            </div>
                ...
                <div class="row g-0">
                    <div class="col-lg-8 pe-lg-2">
                        <!-- 导出表数据 -->
                        <div class="card mb-3">
                        <div class="card-header"><h5>导出表数据</h5></div>
                        <div class="card-body">
                            <input v-model="tableName" placeholder="表名" class="form-control mb-2"/>
                            <div class="row gx-2 mb-2">
                            <div class="col">
                                <select v-model="tableFormat" class="form-select">
                                <option value="csv">CSV</option>
                                <option value="excel">Excel</option>
                                </select>
                            </div>
                            <div class="col">
                                <input v-model="tableFilename" placeholder="导出文件名（可选）" class="form-control"/>
                            </div>
                            </div>
                            <button class="btn btn-primary" @click="handleExportTable">导出表</button>
                        </div>
                        </div>
                        <!-- 预览 & 导出：数据清洗 -->
                        <div class="card mb-3">
                        <div class="card-header"><h5>数据清洗</h5></div>
                        <div class="card-body">
                            <div class="mb-2">
                            <select v-model="cleanFormat" class="form-select">
                                <option value="csv">CSV</option>
                                <option value="excel">Excel</option>
                                <option value="json">JSON</option>
                            </select>
                            </div>
                            <input v-model="targetColumn" placeholder="目标清洗列" class="form-control mb-2"/>
                            <input v-model="dependentColumnsText" placeholder="依赖列（逗号分隔）" class="form-control mb-2"/>
                            <button class="btn btn-outline-primary me-2" @click="handleCleanPreview">预览清洗</button>
                            <button class="btn btn-primary" @click="handleCleanExport">清洗并导出</button>
                        </div>
                        </div>
                        <!-- 预览 & 导出：去重 -->
                        <div class="card mb-3">
                        <div class="card-header"><h5>数据去重</h5></div>
                        <div class="card-body">
                            <div class="mb-2">
                            <select v-model="cleanFormat" class="form-select">
                                <option value="csv">CSV</option>
                            ...
                            <input v-model="duplicatePKText" placeholder="主键列（逗号分隔）" class="form-control mb-2"/>
                            <input v-model="duplicateColsText" placeholder="查重列（逗号分隔）" class="form-control mb-2"/>
                            <button class="btn btn-outline-primary me-2" @click="handleRemoveDupPreview">预览去重</button>
                            <button class="btn btn-primary" @click="handleRemoveDupExport">去重并导出</button>
                        </div>
                        </div>
                        ...
                        <div class="card-body bg-body-tertiary">
                            ...
                            <div class="fallback"><input name="file" type="file" multiple="multiple" @change="handleFileChange"/></div>
                            <div class="dz-message" data-dz-message="data-dz-message"> <img class="me-2" src="../../assets/img/icons/cloud-upload.svg" width="25" alt="" />请在此添加你想要处理的文件</div>
                        ...
                        <div class="card mb-lg-0">
                            ...
                            <div class="card-body bg-body-tertiary">
                                ...
                                <div class="form-check custom-checkbox mb-0">
                                    ...
                                    <label class="form-label mb-0" for="customRadio6">数据清洗</label>
                                </div>
                                <div class="form-check custom-checkbox mb-0">
                                    <input class="form-check-input" id="customRadio6" type="checkbox" />
                                    <label class="form-label mb-0" for="customRadio6">数据去重</label>
                                </div>
                                <div class="border-bottom border-dashed my-3"></div>
                                <button class="btn btn-outline-primary me-2" @click="handleCleanPreview">预览清洗</button>
                                <button class="btn btn-primary mb-2" @click="handleCleanExport">清洗并导出</button>
                                <button class="btn btn-outline-primary me-2" @click="handleRemoveDupPreview">预览去重</button>
                                <button class="btn btn-primary" @click="handleRemoveDupExport">去重并导出</button>
                            </div>
                            <div class="card-bottom">
                                <button class="btn btn-link text-600 btn-sm btn-reveal" type="button" @click="submitFile">提交</button>
                            </div>
                        </div>
                    </div>
                </div>
