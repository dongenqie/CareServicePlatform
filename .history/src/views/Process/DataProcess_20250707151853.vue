...
async function handleCleanPreview() {
  ...
  try {
    ...
    console.log('cleanData 接口完整返回：', response)
    if (response.data.code === 200) {
      previewData.value = response.data.data
      console.table(previewData.value)
      // TODO: 你自己把 previewData 渲染到表格
    } else {
      alert(response.data.message || '清洗预览失败')
    }
  } catch (err) {
    console.error(err)
