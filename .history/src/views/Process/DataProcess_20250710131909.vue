...
// 声明组件名
defineOptions({ name: 'DataProcess' })
...
async function handleCleanExport() {
  ...
  try {
    await cleanExport(requestData);
  } catch (err) {
    console.error(err);
    alert('清洗并导出请求失败，请检查控制台');
  }
}
