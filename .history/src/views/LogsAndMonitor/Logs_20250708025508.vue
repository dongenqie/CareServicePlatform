...
// 拉取列表
function fetchList() {
  getApplications(1, 10)  // 获取列表数据
    .then(response => {
      if (response && response.data && response.data.records.length > 0) {
        applications.value = response.data.records;  // 如果有数据，则填充列表
      } else {
        applications.value = [];  // 如果没有数据，则初始化为空
        console.warn("没有数据加载");
      }
    })
    .catch(error => {
                            ...
                            <tr v-for="app in applications" :key="app.id">
                              ...
                              <td class="align-middle client white-space-nowrap pe-3 pe-xxl-4 ps-2">
                                <div class="d-flex align-items-center gap-2 position-relative">
                                  ...
                                  <h6 class="mb-0">{{ app.username }}</h6>
                                </div>
                              </td>
                              <td class="align-middle subject py-2 pe-4"><a class="fw-semi-bold" href="../app/support-desk/tickets-preview.html">{{ app.reason }}</a></td>
                              <td class="align-middle client white-space-nowrap pe-3 pe-xxl-4 ps-2">
                                <div class="d-flex align-items-center gap-2 position-relative">
                                  <h6 class="mb-0"><a class="stretched-link text-900" href="">{{ new Date(app.applyTime).toLocaleString() }}</a></h6>
                                </div>
                              </td>
                              <td class="align-middle status fs-9 pe-4">
                                <small class="badge rounded"
                                      :class="{
                                        'badge-subtle-danger': app.status === '未处理',
                                        'badge-subtle-warning': app.status === '进行中',
                                        'badge-subtle-success': app.status === '已处理'
                                      }">
                                  {{ app.status }}
                                </small>
                              </td>
                              <td class="align-middle agent">
                                <button class="btn btn-falcon-success btn-sm me-1"
                                        @click="openModal(app.id, true)">
                                  通过
                                </button>
                                <button class="btn btn-falcon-danger btn-sm"
                                        @click="openModal(app.id, false)">
                                  拒绝
                                </button>
                              </td>
                              <td class="align-middle client white-space-nowrap pe-3 pe-xxl-4 ps-2">
                                {{ app.handleTime ? new Date(app.handleTime).toLocaleString() : '---' }}
                              </td>
                            </tr>
                          </tbody>
                        </table>
