...
async function sendMessage() {
  ...
  // 保存用户输入的消息
  const userMessage = { type: 'user', text: aiInput.value, time: new Date().toLocaleTimeString() };
  chatHistory.value.push(userMessage);
  
  // 发送请求到AI并获取回复
  try {
    const aiReply = "这是AI的回复";  // 替换为实际的API请求
    const aiMessage = { type: 'ai', text: aiReply, time: new Date().toLocaleTimeString() };
    chatHistory.value.push(aiMessage);
  } catch (e) {
    chatHistory.value.push({ type: 'ai', text: '发生错误，请重试', time: new Date().toLocaleTimeString() });
  } finally {
            ...
            <div class="chat-content-body" style="display: inherit;">
              <div class="conversation-info" data-index="0">
                <div class="h-100 overflow-auto scrollbar">
                  <div class="chat-content-scroll-area scrollbar" id="chat-history">
                    <!-- 显示历史聊天记录 -->
                    <div v-for="(msg, index) in chatHistory" :key="index" class="d-flex p-3">
                      <div :class="{'flex-1 d-flex justify-content-end': msg.type === 'user', 'd-flex': msg.type !== 'user'}">
                        <div class="w-100 w-xxl-75">
                          <div class="hover-actions-trigger d-flex flex-end-center">
                            <div :class="{'bg-primary text-white p-2 rounded-2 chat-message': msg.type === 'user', 'chat-message bg-200 p-2 rounded-2': msg.type !== 'user'}">
                              <p class="mb-0">{{ msg.text }}</p>
                            </div>
                          </div>
                          <div class="text-400 fs-11 text-end" v-if="msg.time">{{ msg.time }}</div>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <!-- 发送消息区域 -->
              <form @submit.prevent="sendMessage" class="chat-editor-area">
                <div class="emojiarea-editor outline-none scrollbar" contenteditable="true" placeholder="Type your message" v-model="aiInput"></div>
                <button class="btn btn-sm btn-send shadow-none" type="submit" :disabled="isChatLoading">
                  {{ isChatLoading ? '加载中…' : '发送' }}
                </button>
              </form>
            </div>
