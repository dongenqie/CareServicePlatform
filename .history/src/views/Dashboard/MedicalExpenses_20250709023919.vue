...
const fetchMedicalCosts = async () => {
  try {
    ...
    console.log("获取到的医疗费用数据:", response);  // 打印响应数据以检查格式
    console.log("获取到的医疗费用数据:", response.data);  // 打印响应数据以检查格式
    console.log("获取到的医疗费用数据长度:", response.data.length);  // 打印响应数据以检查格式

    if (!response || !response.data || response.data.length === 0) {
      console.error("返回的数据为空或格式错误");
      return;
    }
    medicalCostData.value = formatData(response.data);  // 格式化数据
...
const formatData = (data) => {
  ...
  // 遍历后端数据，根据 level 分类数据
  data.forEach(item => {
    if (item.level && dataset[item.level]) {
      dataset[item.level].push(item.hospitalTotalCost);
    }
  });
