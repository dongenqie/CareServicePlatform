...
const fetchMedicalCosts = async () => {
  try {
    ...
    if (response && response.data) {
      ...
      console.log("formatted data:", medicalCostData.value);
...
const formatData = (data) => {
  console.log("Original data:", data);  // 打印原始数据，确保数据格式正确
  ...
  // 初始化数据集
  const dataset = {
    total: new Array(8).fill(0),
    committee: new Array(8).fill(0),
    province: new Array(8).fill(0),
    prefectureLevel: new Array(8).fill(0),
    countyLevel: new Array(8).fill(0),
    county: new Array(8).fill(0)
  };
  ...
  // 遍历数据，根据 level 分类数据
  data.forEach(item => {
    console.log("item:", item);  // 打印每个 item，查看其结构
    if (item.level && dataset[item.level]) {
      console.log("item.level", item.level, "dataset[item.level]:", dataset[item.level]);
      const yearIndex = years.indexOf(`${item.year}年`);
      if (yearIndex !== -1) {
        // 将数据填充到对应位置
        dataset[item.level][yearIndex] = item.hospitalTotalCost;
      }
    }
  });
