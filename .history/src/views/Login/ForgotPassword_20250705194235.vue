...
import { resetPassword as apiResetPassword } from '@/api/user.js'
...
async function handleResetPassword() {
  ...
  // 基本验证
  if (!form.value.username || !form.value.email || !form.value.newPassword || !form.value.confirmPassword) {
  ...
  try {
    ...
    if (result.success) {
      successMsg.value = result.message || '密码重置成功，请重新登录'
      // 可以选择跳转到登录页
      setTimeout(() => {
    ...
    <main class="main" id="top">
      <div class="container" data-layout="container">
        <div class="row flex-center min-vh-100 py-6 ">
          <div class="col-sm-10 col-md-8 col-lg-6 col-xl-5 col-xxl-4"><a class="d-flex flex-center mb-4" href=""><img class="me-2" src="../../assets/img/logo.png" alt="" width="54" /><span class="font-sans-serif text-primary fw-bolder fs-4 d-inline-block">健康大数据中心</span></a>
            <div class="card">
              <div class="card-body p-4 p-sm-5">
                <h5 class="mb-0 text-center">忘记密码?</h5><h6 class=" text-center" style="color: #748194;">输入用户名和正确邮箱地址即可重置密码</h6>
                <!-- 错误和成功信息 -->
                ...
                <form @submit.prevent="handleResetPassword" class="mt-4">
                  <div class="mb-1">
                    ...
                    <input class="form-control" type="text" id="card-name" v-model="form.username" placeholder="Username" />
                  </div>
                  <div class="mb-1">
                    <label class="form-label" for="card-email">地址</label>
                    <input class="form-control" type="email"  id="card-email" v-model="form.email" placeholder="Email address" />
                  </div>
                  <div class="mb-1">
                    <label class="form-label" for="card-password">新密码</label>
                    <input class="form-control" type="password" id="card-password" v-model="form.newPassword" placeholder="New Password" />
                  </div>
                  <div class="mb-1">
                    <label class="form-label" for="card-confirm-password">确认密码</label>
                    <input class="form-control" type="password" id="card-confirm-password" v-model="form.confirmPassword" placeholder="Confirm Password" />
                  </div>
                  <button class="btn btn-primary d-block w-100 mt-3" type="submit" name="submit">修改密码</button>
                </form><a class="fs-10 text-600 text-center" href="">遇到问题无法修改密码？联系管理员</a>
              </div>
            </div>
          </div>
        </div>
