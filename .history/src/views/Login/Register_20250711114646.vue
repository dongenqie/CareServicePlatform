...
async function handleRegister() {
  ...
  // 基本表单验证
  if (!form.value.username || !form.value.password || !form.value.email || !form.value.phone) {
                ...
                <div class="row g-0 h-100">
                  <div class="col-md-5 text-center bg-card-gradient">
                    ...
                    <div class="mt-3 mb-4 mt-md-4 mb-md-5" data-bs-theme="light">
                      <p class="pt-3 text-white">已有账号?<br><a class="btn btn-outline-light mt-2 px-4" @click="goToLogin" href="">去登录</a></p>
                    </div>
                  </div>
                  <!-- 注册表单 -->
                    ...
                    <div class="p-4 p-md-5 flex-grow-1">
                      ...
                      <div v-if="errorMsg" class="alert alert-danger py-2">{{ errorMsg }}</div>
                                    ...
                                    <div class="tab-content">
                                        ...
                                        <div class="tab-content border-top-0 p-3" id="myTabContent">
                                            <div class="tab-pane fade show active" id="tab-home" role="tabpanel" aria-labelledby="home-tab">
                                                <div class="mb-1">
                                                  <label class="form-label" for="card-name">用户名</label>
                                                  <input class="form-control" v-model="form.username" type="text" autocomplete="username" id="card-name" />
                                                </div>
                                                <div class="mb-1">
                                                  <label class="form-label" for="card-email">邮箱</label>
                                                  <input class="form-control" v-model="form.email" type="email" autocomplete="email" id="card-email" /></div>
                                                <div class="mb-1">
                                                  <label class="form-label" for="card-phone">手机号</label>
                                                  <input class="form-control" v-model="form.phone" type="tel" autocomplete="tel" id="card-phone" /></div>
                                                <div class="mb-2">
                                                  <label class="form-label" for="reg-role">角色</label>
                                                  <select id="reg-role" v-model="form.role" class="form-control form-select form-select-sm audience-select-menu">
                                                    <option v-for="role in roles" :key="role.value" :value="role.value">{{ role.label }}</option>
                                                  </select>
                                                </div>
                                                <div class="row gx-2">
                                                    <div class="mb-0 col-sm-6">
                                                      <label class="form-label" for="card-password">密码</label>
                                                      <input class="form-control" type="password" autocomplete="new-password" id="card-password" v-model="form.password" /></div>
                                                    <div class="mb-0 col-sm-6">
                                                      <label class="form-label" for="card-confirm-password">确认密码</label>
                                                      <input class="form-control" type="password" autocomplete="new-password" id="card-confirm-password" v-model="form.confirmPassword" /></div>
                                                </div>
                                            </div>
                                        </div>
                                        </div>
                        ...
                        <div class="form-check mt-2">
                          ...
                          <label class="form-label" for="card-register-checkbox">我接受<a href="#!">条款</a>和<a class="white-space-nowrap" href="#!">约定</a></label></div>
                        <div class="mb-3">
                          <button class="btn btn-primary d-block w-100 mt-3" type="submit" name="submit">注册</button></div>
                      </form>
                    </div>
                  </div>
                </div>
