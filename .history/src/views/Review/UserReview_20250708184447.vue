...
const fetchResearcherDetail = async (userId) => {
  try {
    const data = await getResearcherDetail(userId);
    selectedResearcher.value = data.user;
  } catch (err) {
    console.error('获取研究员详情失败', err);
    alert('获取研究员详情失败');
  }
};
                  ...
                  <div class="table-responsive scrollbar">
                    <table class="table fs-10 mb-0">
                      ...
                      <tbody class="list" id="table-recent-leads-body">
                        <tr v-for="(researcher, index) in researcherList" :key="index" class="hover-actions-trigger btn-reveal-trigger hover-bg-100">
                          ...
                          <td class="align-middle text-center white-space-nowrap"><small class="badge fw-semi-bold rounded-pill status badge-subtle-success">{{ researcher.status === 1 ? '在线' : '离线' }}</small></td>
