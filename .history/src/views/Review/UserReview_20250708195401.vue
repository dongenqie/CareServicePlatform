...
import { getResearcherList, getResearcherDetail, updateResearcher, deleteResearcher } from '@/api/auditor'; // 新增 API 函数
// 声明组件名
defineOptions({ name: 'Dashboard' })

// 从 localStorage 取得当前用户角色
const role = ref(localStorage.getItem('userRole') || '')
const token = localStorage.getItem('token'); // 获取审核员的JWT Token
...
const goToPersonalCenter = (userId, role) => {
  router.push({ name: 'PersonalCenter', params: { id: userId, role: role } });
};

// 路由跳转到个人中心页面，传递研究员的 id
const goToXPersonalCenter = (userId) => {
  router.push({ name: 'PersonalCenter', params: { id: userId } });
};

// 获取研究员列表
const researcherList = ref([]);
const fetchResearcherList = async () => {
  try {
    const data = await getResearcherList({ current: 1, size: 10 });
    console.log('获取到的研究员数据:', data);
    if (data && data.records) {
      researcherList.value = data.records;
    } else {
      researcherList.value = [];  // 如果没有返回数据，确保列表为空
    }
    console.log('研究员列表:', researcherList.value);

    // 重新初始化 list.min.js 插件
    if (window.List) {
      const list = new window.List('TableCrmRecentLeads', {
    ...
    console.error('获取研究员列表失败', err);
    alert('获取研究员列表失败');
    researcherList.value = [];  // 错误时设置为空列表
  }
};

// 修改研究员信息
const modifyResearcher = async () => {
  try {
    const updatedData = {
      username: selectedResearcher.value.username,
      email: selectedResearcher.value.email,
      phone: selectedResearcher.value.phone,
      status: selectedResearcher.value.status,
    };
    const response = await updateResearcher(selectedResearcher.value.id, updatedData);
    if (response.success) {
      alert('修改成功');
      fetchResearcherList();  // 刷新列表
    } else {
      alert('修改失败');
    }
  } catch (err) {
...
// 删除研究员
const removeResearcher = async (userId) => {
  // 显示删除确认提示框
  const confirmDelete = window.confirm('确认删除该研究员吗？');
  if (!confirmDelete) return; // 如果用户点击取消，直接返回

  try {
    const response = await deleteResearcher(userId);
    if (response.success) {
      alert('删除成功');
      fetchResearcherList();  // 刷新列表
    } else {
                  ...
                  <div class="card bg-body-tertiary dark__bg-opacity-50 shadow-none">
                    ...
                    <div class="d-flex align-items-center z-1 p-0">
                      ...
                      <div class="ms-n3">
                        ...
                        <h4 class="mb-0 text-primary fw-bold">健康大数据-</h4><span class="text-info fw-medium">用户审核中心</span>
                      </div>
              ...
              <div class="card" id="TableCrmRecentLeads" data-list='{"valueNames":["name","email","status"],"page":8,"pagination":true}'>
                <div class="card-header d-flex flex-between-center flex-wrap gap-2 py-2">
                  ...
                  <div id="table-recent-leads-replace-element">
                    <div class="dropdown font-sans-serif btn-reveal-trigger"><button class="btn btn-link text-600 btn-sm dropdown-toggle dropdown-caret-none btn-reveal" type="button" id="recent-leads-header-dropdown" data-bs-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false"><span class="fas fa-ellipsis-h fs-11"></span></button>
                      <div class="dropdown-menu dropdown-menu-end border py-2" aria-labelledby="recent-leads-header-dropdown"><a class="dropdown-item" href="#!">导出数据</a><a class="dropdown-item" href="#!">Export</a>
                    ...
                    <table class="table fs-10 mb-0">
                      <thead class="bg-200">
                        <tr>
                          ...
                          <th class="text-800 sort align-middle text-center" data-sort="name">用户名</th>
                          <th class="text-800 sort align-middle text-center" data-sort="email">邮箱</th>
                          <th class="text-800 sort align-middle text-center" data-sort="phone">电话</th>
                          <th class="text-800 sort align-middle text-center" data-sort="status">状态</th>
                          <th class="text-800 sort align-middle text-center">操作</th>
                        </tr>
                      </thead>
                      <tbody class="list" id="table-recent-leads-body">
                        <tr v-for="(researcher, index) in researcherList" :key="researcher.id"  class="hover-actions-trigger btn-reveal-trigger hover-bg-100">
                          ...
                          <td class="align-middle white-space-nowrap" >
                            <a class="text-800" href="" @click="goToXPersonalCenter(researcher.id)">
                              <div class="d-flex align-items-center">
                                <div class="avatar avatar-xl">
                                  <img class="rounded-circle" src="../../assets/img/profile/1.jpg" alt="" />
                                </div>
                                <h6 class="mb-0 ps-2 name">{{ researcher.username }}</h6>
                              </div>
                            </a>
                          </td>
                          <td class="align-middle text-center white-space-nowrap text-primary email">{{ researcher.email }}</td>
                          <td class="align-middle text-center white-space-nowrap">{{ researcher.phone }}</td>
                          <td class="align-middle text-center white-space-nowrap">
                            <small class="badge fw-semi-bold rounded-pill status badge-subtle-success">
                              {{ researcher.status === 1 ? '在线' : '离线' }}
                            </small>
                          </td>
                          <td class="align-middle text-center white-space-nowrap text-end position-relative">
                            <div class="hover-actions bg-100">
                              <!--查看研究员信息按钮-->
                              <button class="btn icon-item rounded-3 me-2 fs-11 icon-item-sm" @click="fetchResearcherDetail(researcher.id)"><span class="far fa-edit"></span></button>
                              <!--删除研究员信息按钮-->
                              <button class="btn icon-item rounded-3 me-2 fs-11 icon-item-sm" @click="removeResearcher(researcher.id)"><span class="far fa-comment"></span></button>
                            </div>
                            <div class="dropdown font-sans-serif btn-reveal-trigger"><button class="btn btn-link text-600 btn-sm dropdown-toggle dropdown-caret-none btn-reveal-sm transition-none" type="button" id="crm-recent-leads-0" data-bs-toggle="dropdown" data-boundary="viewport" aria-haspopup="true" aria-expanded="false"><svg class="svg-inline--fa fa-ellipsis-h fa-w-16 fs-11" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="ellipsis-h" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" data-fa-i2svg=""><path fill="currentColor" d="M328 256c0 39.8-32.2 72-72 72s-72-32.2-72-72 32.2-72 72-72 72 32.2 72 72zm104-72c-39.8 0-72 32.2-72 72s32.2 72 72 72 72-32.2 72-72-32.2-72-72-72zm-352 0c-39.8 0-72 32.2-72 72s32.2 72 72 72 72-32.2 72-72-32.2-72-72-72z"></path></svg><!-- <span class="fas fa-ellipsis-h fs-11"></span> Font Awesome fontawesome.com --></button>
                ...
                <div class="card-footer">
                  <div class="row align-items-center">
                    ...
                    <div class="col">
                      <p class="mb-0 fs-10"><span class="d-none d-sm-inline-block me-2" data-list-info="data-list-info">1 to 8 of 13</span></p>
                    </div>
