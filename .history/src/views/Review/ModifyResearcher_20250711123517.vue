...
import { getResearcherDetail, updateResearcher } from '@/api/auditor'
...
const fetchResearcherInfo = async () => {
  try {
    const response = await getResearcherDetail(userId);
    console.log("获取到的研究员数据:", response);
    if (response && response.user) {  // 通过检查 user 字段确保有数据
      username.value = response.user.username;
      email.value = response.user.email;
      phone.value = response.user.phone;

      console.log(username.value, email.value, phone.value, role.value);
    }
  } catch (err) {
    alert('获取研究员信息失败');
    console.error(err);
...
const handleSubmit = async () => {
  ...
  try {
    const result = await updateResearcher(userId, updatedData)
    if (result.success) {
      alert('信息更新成功')
    } else {
      alert('信息更新失败')
              ...
              <div class="card mb-3">
                ...
                <div class="card-body bg-body-tertiary">
                  <form class="row g-3" @submit.prevent="handleSubmit">
                    ...
                    <div class="col-lg-6"> 
                        <label class="form-label" for="last-name">角色</label>
                        <input class="form-control" id="last-name" type="text" value="RESEARCHER" readonly />
                    </div>
                    <div class="col-lg-6"> 
