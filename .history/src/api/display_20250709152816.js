...
// 新增：按级别和年份取三条数据
export const getCostDataByLevelAndYears = async (levelZh, yearsArr) => {
  // levelZh: 后端存的中文级别，yearsArr: [2015,2016,…]
  const yearsStr = yearsArr.join(',')
  const resp = await apiClient.get(
    `/api/hospital/hospital/cost/level/${encodeURIComponent(levelZh)}/years/${yearsStr}`
  )
  // 返回：ComprehensiveHospitalCost[]
  return resp.data
}
